...
public class ArticleApi extends BaseApi<Article, Integer> implements IArticleApi
{
	...
	{
		return
		getDao().queryForFirst(getDao().queryBuilder().where().eq(IArticle.Column.source, article.getSource()).and().eq(IArticle.Column.sourceId, article.getSourceId()).prepare()) !=
		null;
	}
}
